...
def main():
    ...
    # Conversation handler para crear repos
    ...
    # Conversation handler para fork
    fork_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(fork_repo_handler, pattern='^fork_repo$')],
        states={
            FORK_URL: [MessageHandler(filters.TEXT & ~filters.COMMAND & allowed, fork_repo_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )

    # Conversation handler para upload
    ...
    # Conversation handler para descarga ZIP
    zip_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(download_zip_handler, pattern='^download_zip$')],
        states={
            ZIP_REPO: [MessageHandler(filters.TEXT & ~filters.COMMAND & allowed, download_zip_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )

    # Conversation handler para AI
    ai_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(ai_chat_handler, pattern='^ai_chat$')],
        states={
            AI_MESSAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND & allowed, ai_chat_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )

    # Conversation handler para búsqueda
    search_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(search_repos_handler, pattern='^search_repo$')],
        states={
            SEARCH_QUERY: [MessageHandler(filters.TEXT & ~filters.COMMAND & allowed, search_repos_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )

    # Conversation handler para URL download
    url_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(url_download_handler, pattern='^url_download$')],
        states={
            URL_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND & allowed, url_download_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )

    # Conversation handler para edit
    ...
    app.add_handler(CommandHandler('start', unauthorized))
